...
class TNFModelTester:
        ...
        try:
            ...
            for key in expected_loss_keys:
                if key not in loss_dict:
                    self.log_test("Loss computation", False, f"Missing loss key: {key}")
                    return False
            
            self.log_test("Loss computation", True)
    ...
    def test_training_components(self, quick: bool = False):
        ...
        try:
            ...
            with torch.no_grad():
                ...
                metrics = calculate_metrics(targets_np, probs, threshold=0.5)
                self.log_test("Metrics calculation", True, f"F1: {metrics['f1']:.3f}")
    ...
    def test_gpu_compatibility(self):
        ...
        try:
            ...
            if gpu_memory < 4.0:
                self.log_test("GPU memory warning", False, f"Low GPU memory ({gpu_memory:.1f}GB). Consider reducing batch size.", warning=True)
            elif gpu_memory < 8.0:
                self.log_test("GPU memory recommendation", True, "Moderate GPU memory. Batch size 32-64 recommended.", warning=True)
            else:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Test MM-TNF model components",
    ...
    parser.add_argument(
        "--gpu-only", 
        action="store_true", 
        help="Only run GPU-specific tests"
    )
