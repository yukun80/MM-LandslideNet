...
class SimpleKFoldTrainer:
    ...
    def _train_single_fold(self, fold_idx: int, kfold_wrapper) -> Dict[str, Any]:
        ...
        # 只在配置中设置基本类型参数
        trainer_config["deterministic"] = True  # 确保可重现性
    ...
    def _create_fold_callbacks(self, fold_idx: int) -> List[pl.Callback]:
        """
        为单折创建回调函数 - 正确版本：区分不同回调的参数

        这个修复版本正确地处理了不同回调函数的参数差异。
        就像为不同的工具使用正确的操作方法：
        - 锤子用来敲击，不能拧螺丝
        - 螺丝刀用来拧螺丝，不能敲击

        关键学习点：
        - ModelCheckpoint没有min_delta参数
        - EarlyStopping才有min_delta参数
        - 每个类的API文档都需要仔细查阅
        """
        callbacks = []

        # 模型检查点回调 - 注意：ModelCheckpoint没有min_delta参数！
        if "callbacks" in self.config and "model_checkpoint" in self.config["callbacks"]:
            ...
            # 注意：我们不读取min_delta，因为ModelCheckpoint不支持这个参数
            logger.info(f"🔧 ModelCheckpoint config for fold {fold_idx}:")
            logger.info(f"   - monitor: {monitor_metric}")
            logger.info(f"   - mode: {mode}")
            logger.info(f"   - save_top_k: {save_top_k}")
            logger.info(f"   - ModelCheckpoint不使用min_delta参数（这是EarlyStopping的参数）")
        ...
        if "callbacks" in self.config and "early_stopping" in self.config["callbacks"]:
            ...
            # EarlyStopping的有效参数（包括min_delta）
            ...
            # 关键修复：只有EarlyStopping才使用min_delta参数
            min_delta = early_stop_config.get("min_delta", 0.0)

            logger.info(f"🔧 EarlyStopping config for fold {fold_idx}:")
            logger.info(f"   - monitor: {monitor_metric}")
            logger.info(f"   - patience: {patience}")
            logger.info(f"   - min_delta: {min_delta} (只有EarlyStopping使用此参数)")
            ...
            early_stopping = EarlyStopping(
                ...
                patience=patience,
                verbose=verbose,
                min_delta=min_delta,  # 只在EarlyStopping中使用min_delta
            )
